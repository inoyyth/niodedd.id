...
class Subcategory_article extends MX_Controller{
    ...
    /* function index($id){
        ...
        //tamplikan data
        $data['total_data']=$this->db->query("select * from article where id_subcategory='$id' and status='Y' order by id desc")->num_rows();
        $data['datax'] = $this->db->query("select * from article where id_subcategory='$id' and status='Y' order by id desc limit ".$pg.",".$config['per_page']."")->result_array();
        $data['title']= $this->uri->segment(4);
        $data['sub_category']=$this->db->query("select * from article_subcategory where id='$id'")->row_array();
