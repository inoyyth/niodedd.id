<form method="post" action="<?=base_url();?>menu/update_proses" enctype="multipart/form-data" >
...
<div class="panel panel-success">
    ...
    <div class="panel-body" id="panelx">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="menu_parent" required>
                ...
                <option style="color: #000;" value="<?="1-".$menux->id;?>" <?=$cek1;?>><?=$menux->menu_name;?></option>
                    ...
                    $idx2=$list->id;
                    //echo $mid."<br>".$idx;
                    $sq=$this->db->query("select * from menu where menu_parent_id='$mid' and id not in ($idx2) order by menu_position asc");
                    foreach($sq->result_array() as $daty){ if($idx==$daty['id']){$cek2="selected";}else{$cek2="";}?>
                    <option style="color: #0063DC;" value="<?="2-".$daty['id'];?>" <?=$cek2;?>> - <?=$daty['menu_name'];?></option>
                        <?php
                        $sqx=$this->db->query("select * from menu where menu_parent_id='".$daty['id']."' and id not in ($idx2) order by menu_position asc");
                        foreach($sqx->result_array() as $datx){ if($idx==$datx['id']){$cek3="selected";}else{$cek3="";}?>
                        <option style="color: #86d5f8;" value="<?="3-".$datx['id'];?>" <?=$cek3;?>>  &nbsp;&nbsp; - - <?=$datx['menu_name'];?></option>
                            <?php
                            $sqc=$this->db->query("select * from menu where menu_parent_id='".$datx['id']."' and id not in ($idx2) order by menu_position asc");
                            foreach($sqc->result_array() as $datc){ if($idx==$datc['id']){$cek4="selected";}else{$cek4="";}?>
                            <option style="color: #800;" value="<?="4-".$datc['id'];?>" <?=$cek4;?>>  &nbsp;&nbsp; - - - <?=$datc['menu_name'];?></option>
                                <?php
                                $sqb=$this->db->query("select * from menu where menu_parent_id='".$datc['id']."' and id not in ($idx2) order by menu_position asc");
                                foreach($sqb->result_array() as $datb){ if($idx==$datb['id']){$cek5="selected";}else{$cek5="";}?>?>
                                <option style="color: #f0ad4e;" value="<?="5-".$datb['id'];?>" <?=$cek5;?>>  &nbsp;&nbsp; - - - - <?=$datb['menu_name'];?></option>
                                    <?php
                                    $sqd=$this->db->query("select * from menu where menu_parent_id='".$datb['id']."' and id not in ($idx2) order by menu_position asc");
                                    foreach($sqd->result_array() as $datd){ if($idx==$datd['id']){$cek6="selected";}else{$cek6="";}?>?>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="menu_value" autocomplete="off" name="menu_value"  required/><a href="#" id="linkx">Search</a>
