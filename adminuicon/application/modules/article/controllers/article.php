...
class Article extends MX_Controller{
    ...
    function index(){
        ...
        $config['total_rows'] = $this->db->query("select a.*,b.article_category_name,c.article_subcategory_name 
                                                from article a 
                                                left join article_category b on a.id_category=b.id 
                                                left join article_subcategory c on a.id_subcategory=c.id 
    ...
    public function search()
	{
            if($_POST){
                ...
                $subcategory_sr = $this->main_model->handler0('subcategory_sr',$this->input->get_post('subcategory_sr', TRUE));
            }
            //echo $id_sr,$name_sr,$status_sr;
            $limit = ($this->uri->segment(3) > 0)?$this->uri->segment(3):0;

            $config['base_url'] = base_url() .'article/search';
            $config['total_rows'] = $this->db->query("select a.*,b.article_category_name,c.article_subcategory_name from article a left join article_category b on a.id_category=b.id left join article_subcategory c on a.id_subcategory=c.id where a.id LIKE '%$id_sr%' and a.article_name LIKE '%$name_sr%' and a.status LIKE '%$status_sr%' and b.article_category_name LIKE '%$category_sr%' and c.article_subcategory_name LIKE '%$subcategory_sr%' order by a.id desc")->num_rows();
    ...
    function update($id,$page){
        $data['list_category']=$this->db->query("select * from article_category where status='Y'")->result();
        //$data['list_subcategory']=$this->db->query("select * from article_subcategory where status='Y'")->result();
    ...
    function update_proses(){
        ...
        $data=array("id_category"=>$cat,"id_subcategory"=>$subcat,"article_name"=>$name,"article_description"=>$description,"status"=>$status,"article_image"=>$thumbnail,"sys_update_user"=>$session_data['user_id'],"sys_update_date"=>$datetime,"index_article"=>$index_article);
