                    ...
                    ?>
                        <tr>
                            ...
                            <td style="text-align: center;">
                                ...
                                <button id="email" onclick="send_mail('<?=$data->id;?>');" type="button" class="btn btn-default btn-xs"><i class="fa envelope-o"> Send</i></button>
                            </td>
                        </tr>
                      <?php }} ?>
                    </tbody>
                    <tfoot>
                        <tr>
                            <form method="post" action="<?=base_url();?>newsletter/search" id="form1"/>
                                ...
                                <td><input name="title_sr" class="form-control" style="width: 100%;" type="text" onkeyup="javascript:if(event.keyCode == 13){coba('form1');}else{return false;};"/></td>
                                <td><input name="count_sr" class="form-control" style="width: 100%;" type="text" onkeyup="javascript:if(event.keyCode == 13){coba('form1');}else{return false;};"/></td>
                                <td></td>
                            </form>
                        </tr>
                    </tfoot>
            ...
            if(confirm('Sure Send This Newsletter ?')){
                $.ajax({
                    ...
                    success: function (data) {
                        window.location.replace('<?=base_url();?>newsletter/index/<?=($this->uri->segment(3)) ? $this->uri->segment(3) : 0 ;?>');
                    }
                });
            }
        }
